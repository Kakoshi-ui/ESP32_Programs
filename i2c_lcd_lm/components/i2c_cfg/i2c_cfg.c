...
void init_i2c(void){
    ...
    i2c_config_t i2c_setup = {
        ...
        .sda_io_num = GPIO_NUM_8,      //! i2c_set_pin(882): scl and sda gpio numbers are the same
        .scl_io_num = GPIO_NUM_9,      //! for the esp32 wrooom 32
        .sda_pullup_en = GPIO_PULLUP_ENABLE,
        .scl_pullup_en = GPIO_PULLUP_ENABLE,
        .master.clk_speed = 100000,
        .clk_flags = 0
    };
    err = i2c_param_config(I2C_MODE_MASTER,&i2c_setup);
    cfg_err = i2c_driver_install(I2C_NUM_0,I2C_MODE_MASTER,0,0,0);
    if(err != 0){
...
void lcd_i2c_write_cmd(char cmd_reg){
    ...
    //? i2c large method install:
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);

    i2c_master_write_byte(cmd,(PCF8574_ADDR<<1)|I2C_MASTER_WRITE,true);
    i2c_master_write(cmd,cmd_buff,4,1);

    i2c_master_stop(cmd);
    //! error command::::
    err = i2c_master_cmd_begin(I2C_NUM_0,cmd,10);
    if(err != 0) ESP_LOGE(TAG,"I2C FAILED SENDING COMMANDS. ERROR: %d",err);
    i2c_cmd_link_delete(cmd);
    //! Maybe not works
    //! err = i2c_master_write_to_device(I2C_NUM_0,PCF8574_ADDR,cmd_buff,4,10);
...
void lcd_i2c_write_data(char dat){
    ...
    //! Error trying to send data, the method i2c_cmd_begin() present TIMEOUT errors
    esp_err_t err;
    uint8_t dat_buff[4];
    dat_buff[1] = (dat & 0xF0)|LCD_RS|LCD_EN|FONT_LIGTH;
    ...
    i2c_master_write_byte(cmd,(PCF8574_ADDR<<1)|I2C_MASTER_WRITE,1);
    i2c_master_write(cmd,dat_buff,4,1);

    i2c_master_stop(cmd);
    err = i2c_master_cmd_begin(I2C_NUM_0,cmd,10);
    if(err != 0) ESP_LOGE(TAG, "I2C FAILED SENDING DATA ERROR: %d",err);
    i2c_cmd_link_delete(cmd);

    //! Maybe not works
    //! err = i2c_master_write_to_device(I2C_NUM_0,PCF8574_ADDR,dat_buff,4,10);     //! ERROR sending data, check out the method
